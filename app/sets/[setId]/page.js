    ...
    return (
      <div className="min-h-screen"><Navbar />
        <main className="container px-4 py-16 text-center">
          ...
          <Link href="/sets" className="text-rift mt-3 inline-block">Back to Sets</Link>
        </main>
      </div>
    )
  }
      ...
      <main>
        <div className={`relative h-72 bg-gradient-to-br ${set.coverGradient} overflow-hidden`}>
          ...
          <div className="container relative h-full px-4 flex flex-col justify-end pb-8">
            ...
            <div className="flex gap-6 mt-4 text-sm">
              <div><span className="text-white/60">Total cards:</span> <span className="font-bold">{set.totalCards}</span></div>
              <div><span className="text-white/60">Available:</span> <span className="font-bold">{cards.length}</span></div>
              <div><span className="text-white/60">Released:</span> <span className="font-bold">{new Date(set.releaseDate).toLocaleDateString('en-US')}</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="container px-4 py-8">
          <div className="flex flex-wrap gap-2 mb-6">
            <button onClick={() => setFilter('all')} className={`px-3 py-1.5 rounded-md text-sm font-medium border ${filter === 'all' ? 'bg-rift border-rift text-white' : 'bg-panel border-white/10 text-muted-foreground'}`}>All ({cards.length})</button>
